...
exports.eventCreate = async (req, res) => {
  try {
    ...
    const newEvent = new Event({
      ...
      eventStreet: req.body.eventStreet,
      zip: req.body.zip,
...
exports.addNewGuests = async (req, res) => {
  ...
  try {
    ...
    if (remainingGuests < 0) {
      ...
      let guestEmailList = [], guestNameList = [], pwdUrl = "";
      const isNewGuest = (guest) => {
        if (event.guests.length > 0) {
          event.guests.map((item, index) => {
            if (item.email.length) { guestEmailList.push(item.email) } else return;
            if (item.name.length) { guestNameList.push(item.name) } else return;
          });
        }

        if ((guestEmailList.length > 0 && guestEmailList.indexOf(guest.email) > -1) || (guestNameList.length > 0 && guest.name != undefined && guestNameList.indexOf(guest.name) > -1)) {
          return false;
        } else {
          return true;
        }
                  ...
                  <div
                      ...
                      <div style='padding: 10px; font-size: 12px;'>
                          <p style='color: #1b1a1a;'>
                              ...
                              <br>
                              <br>
                              <u>${event.hostName ? event.hostName : "admin@mymanager.com"}</u> has invited you to their ${event.title}, in a workspace called
                              Mymanager.
                              <br>
                              <br>
                              <span>Just Click: </span> https://mymanager.com/event-preview/${pwdUrl.length > 0 ? _id + '/' + pwdUrl : _id}
                          </p>
                      </div>
    ...
    if (isNewEmployee) {
      ...
      event.guests = event.guests.map((item, index) => {
        if ((item.name == guestData.name) || (item.email == guestData.email)) {
          ...
          item.status = guestData.replyType;
          return item
        } else return item
      })
    }
